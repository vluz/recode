...
REM   -r 60 - Framerate set to 60 frames per second, change to 30 for 30FPS
...
for %%i in ("*.mp4") do (
    ffmpeg -i "%%i" -hide_banner -vsync 1 -crf 19 -r 60 -c:v libx264 -preset ultrafast -c:a copy "EDIT\%%~ni.mp4"
)
REM END
